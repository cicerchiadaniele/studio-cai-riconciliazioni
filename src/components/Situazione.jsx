...
import Schermata, { Badge, CAMPO } from './Schermata.jsx'
...
export default function Situazione({ elenco, onIndietro }) {
  ...
  return (
    <Schermata
      titolo="Situazione dei conti"
      sotto={`Verde fino a ${GIORNI_OK} giorni, ambra fino a ${GIORNI_AVVISO}, rosso oltre.`}
    ...
    >
      ...
      <ul className="mt-4 flex flex-col gap-2">
        {filtrati.map((c) => (
          <li key={c.id} className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3.5 ring-1 ring-neutral-200">
      ...
      {filtrati.length === 0 && <p className="mt-6 text-center text-neutral-500">Nessun condominio trovato con questo nome.</p>}
    </Schermata>
  )
}
